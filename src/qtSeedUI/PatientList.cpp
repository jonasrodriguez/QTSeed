...
PatientList::PatientList(QObject *parent, std::shared_ptr<IBusiness> &business_logic)
    : QAbstractListModel(parent), patient_count_(0), business_logic_(business_logic)
{
    QObject::connect(business_logic_.get(), SIGNAL(SendPatientList(QVector<Patient>)), this, SLOT(InsertPatients(QVector<Patient>)));
}
...
QVariant PatientList::data(const QModelIndex &index, int role) const {
    ...
    if (index.row() >= patient_list_.size() || index.row() < 0)
        return QVariant();
...
void PatientList::InsertPatients(QVector<Patient> patients) {
  ...
  beginInsertRows(QModelIndex(), patient_list_.size(), patient_list_.size() + patients.size() - 1);
  for (auto i : patients) {
...
void PatientList::refreshPatientList() {
    business_logic_->GetPatientList();
}
